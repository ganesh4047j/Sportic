...
//   }
// }
// lib/category.dart - Optimized Version
// ignore_for_file: avoid_print, unused_element, unused_local_variable
...
class HomePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              // Games List with better performance
              Expanded(
                child: isLargeScreen
                    ? _buildOptimizedGridView(context, ref, matchDetails)
      ...
      bottomNavigationBar: _OptimizedBottomNav(ref: ref),
    );
  }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, rowIndex) {
        return Row(
          children: [
            Expanded(
              child: _buildOptimizedGameCard(
                context,
                ref,
                rowIndex * 2,
                matchDetails[rowIndex * 2],
                true,
              ),
            ),
            const SizedBox(width: 20),
            if (rowIndex * 2 + 1 < imageUrls.length)
              Expanded(
                child: _buildOptimizedGameCard(
                  context,
                  ref,
                  rowIndex * 2 + 1,
                  matchDetails[rowIndex * 2 + 1],
                  true,
                ),
              )
            else
  ...
  Widget _buildOptimizedListView(
    ...
    return ListView.builder(
      ...
      itemCount: imageUrls.length,
      itemBuilder: (context, index) {
        return _buildOptimizedGameCard(
          context,
          ref,
          index,
          matchDetails[index],
  ...
  Widget _buildOptimizedGameCard(
      BuildContext context,
      WidgetRef ref,
      int index,
      Map<String, String> details,
    ...
    return Container(
      margin: EdgeInsets.only(bottom: isGrid ? 20 : (isTablet ? 25 : 20)),
      child: Column(
        children: [
          // Optimized Game Image Card
          Container(
            height: isGrid ? 180 : (isTablet ? 220 : 190),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(20),
            ...
            child: ClipRRect(
              ...
              child: Stack(
                children: [
                  // Game Image with caching
                  Positioned.fill(
                    child: Image.network(
                      imageUrls[index],
                      fit: BoxFit.cover,
                      cacheHeight: 400, // Reduce memory usage
                      cacheWidth: 300,
                      errorBuilder: (context, error, stackTrace) {
                        return Container(
                          decoration: const BoxDecoration(
                            gradient: LinearGradient(
                              colors: [Color(0xffD72664), Color(0xff6624b5)],
                            ),
                          ),
                          child: const Center(
                            child: Icon(
          ...
          // Optimized Dropdown Section
          _OptimizedDropdownSection(
            cardIndex: index,
            details: details,
            isTablet: isTablet,
          ),
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.outfit(
                    color: Colors.white,
                    fontWeight: FontWeight.w800,
                    fontSize: isLargeScreen ? 34 : isTablet ? 30 : 26,
                    letterSpacing: 1.2,
                  ),
                ),
                Text(
...
class _OptimizedBottomNav extends StatelessWidget {
  final WidgetRef ref;

  const _OptimizedBottomNav({required this.ref});
  ...
  void _navigateToScreen(BuildContext context, int index) {
    switch (index) {
      ...
      case 2:
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const CenterLottieScreen()),
        );
        break;
      case 3:
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const TurfHomeScreen()),
        );
        break;
      case 4:
        Navigator.pushReplacement(
...
// Optimized Dropdown Section
class _OptimizedDropdownSection extends ConsumerWidget {
  final int cardIndex;
  final Map<String, String> details;
  final bool isTablet;

  const _OptimizedDropdownSection({
    required this.cardIndex,
    required this.details,
    this.isTablet = false,
  });
      ...
      children: [
        ...
        if (isExpanded)
          Container(
            ...
            padding: EdgeInsets.all(isTablet ? 24 : 20),
            decoration: BoxDecoration(
              color: const Color(0xFF3D1A4A).withOpacity(0.9),
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: Colors.white.withOpacity(0.2),
                width: 1,
            ...
            child: Column(
              ...
              children: [
                ...
                // Details List
                ..._buildDetailsList(details, isTablet),
              ],
            ),
          ).animate().slideY(
            begin: -0.1,
  ...
  List<Widget> _buildDetailsList(Map<String, String> details, bool isTablet) {
    final List<MapEntry<String, String>> entries = [
      MapEntry('Turf name', details['turfName'] ?? 'Unknown'),
      MapEntry('Slot timing', details['timing'] ?? 'TBD'),
      MapEntry('Creator name', details['creator'] ?? 'Anonymous'),
    ];
